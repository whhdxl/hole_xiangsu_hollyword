...
export class VoxelBatches{
  constructor(scene,world,geometry,material){
    ...
    const makeBatch=(ids,capacity,shadows)=>{
      ...
      const batch={mesh,ids:new Uint32Array(capacity),changed:new Uint8Array(Math.ceil(capacity/64)),colorChanged:new Uint8Array(Math.ceil(capacity/64))};batch.ids.set(ids);scene.add(mesh);return batch;
    };
    ...
    this.moving=makeBatch([],world.blocks.length,true);this.moving.mesh.frustumCulled=false;
  ...
  mark(batch,slot,colors=false){batch.changed[slot>>6]=1;if(colors)batch.colorChanged[slot>>6]=1;this.dirty.add(batch);}
  remove(batch,slot,dynamic){
    const last=--batch.mesh.count;this.dirty.add(batch);
    if(slot!==last){const id=batch.ids[last];batch.ids[slot]=id;(dynamic?this.movingSlot:this.slotOf)[id]=slot;batch.mesh.instanceMatrix.array.copyWithin(slot*16,last*16,last*16+16);batch.mesh.instanceColor.array.copyWithin(slot*3,last*3,last*3+3);this.mark(batch,slot,true);}
  }
  transform=(i,b)=>{
    ...
    if(b.state===2){
      if(slot<0){this.remove(origin,this.slotOf[i],false);slot=this.moving.mesh.count++;this.moving.ids[slot]=i;this.movingSlot[i]=slot;
        this.moving.mesh.instanceColor.array.set(this.colors.subarray(i*3,i*3+3),slot*3);this.mark(this.moving,slot,true);
      }
      batch=this.moving;
    }else{
      if(slot>=0){this.remove(this.moving,slot,true);this.movingSlot[i]=-1;this.slotOf[i]=origin.mesh.count++;origin.ids[this.slotOf[i]]=i;origin.mesh.instanceColor.array.set(this.colors.subarray(i*3,i*3+3),this.slotOf[i]*3);this.mark(origin,this.slotOf[i],true);}
      slot=this.slotOf[i];
    }
  ...
  flush(){
    for(const batch of this.dirty){
      ...
      for(let i=0;i<batch.changed.length;i++){if(!batch.changed[i])continue;const start=i;while(i+1<batch.changed.length&&batch.changed[i+1])batch.changed[++i]=0;batch.changed[start]=0;
        const count=Math.min((i+1)*64,batch.mesh.count)-start*64;if(count>0){queueRange(matrix,start*64*16,count*16);}
      }
      for(let i=0;i<batch.colorChanged.length;i++){if(!batch.colorChanged[i])continue;const start=i;while(i+1<batch.colorChanged.length&&batch.colorChanged[i+1])batch.colorChanged[++i]=0;batch.colorChanged[start]=0;
        const count=Math.min((i+1)*64,batch.mesh.count)-start*64;if(count>0)queueRange(colors,start*64*3,count*3);
      }
  ...
  reset(){
    ...
    this.world.blocks.forEach((b,i)=>{const batch=this.batches[this.batchOf[i]],slot=batch.mesh.count++;batch.ids[slot]=i;this.slotOf[i]=slot;writeVoxelMatrix(batch.mesh.instanceMatrix.array,slot*16,b);batch.mesh.instanceColor.array.set(this.colors.subarray(i*3,i*3+3),slot*3);this.mark(batch,slot,true);});
